...
parser.add_argument('--input', required=True, help='Path to data excel file')
args = parser.parse_args()

initial_path = args.input

pathToExel = 'r' + args.input

# ------------------------------ MAIN ------------------------------

# Initialize the trabslator
translator = Translator()

# Read the Excel with all the tweets as a DataFrame
df = pd.read_excel(args.input)

# Copy the DataFrame into a new DataFreame which is going to be modified with the translations
...
for ind in df.index:
    ...
    # If the string is blank, save the row index to delete it later
        ...
        # translations_df.drop([ind], axis=0, inplace=True)
    print(str(ind))
...
final_translation = final_translations.drop(columns=["date"])
